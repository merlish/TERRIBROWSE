...
/**
  A <code>Process</code> is a bit like a parameterless procedure that may be
 ...
 @version $Revision: 471 $ $Date: 2008-09-17 17:37:35 +0100 (Wed, 17 Sep 2008) $
...
/** 
    ...
    then threads are pooled; otherwise whenever a thread is needed
    to execute a process it is obtained from a thread pool.
    <p>
    ...
    If the value of the <code>ox.cso.pool</code> property
    is a number, then the keepalive time for idle pooled threads is
    set to that number of seconds; otherwise a default period is
    used. 
...
{  
   ...
   /** True if threads are being pooled */
   val pooled   = poolTime!=null
   /** keepAlive time */
   val keep = if (pooled && poolTime.matches("[0-9]+")) Integer.parseInt(poolTime) else 10;         
   /** An executor -- pooled if necessary */
   val threads : Executor = if (pooled) new PooledExecutor(keep, false) else new UnpooledExecutor(false)
   /** An executor for daemons -- pooled if necessary */
   val daemons : Executor = if (pooled) new PooledExecutor(keep, true)  else new UnpooledExecutor(true)
