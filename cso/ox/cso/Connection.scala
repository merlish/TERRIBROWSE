...
{ 
  ...
  */
  ...
  /** A <code>ProxyClient(req, rep)</code> is a <code>Client</code> that provides
      requests on <code>req</code> and accepts replies on <code>rep</code>
  */
  ...
  /** A <code>ProxyServer(req, rep)</code> is a <code>Server</code> that accepts
      requests on <code>req</code> and provides replies on <code>rep</code>
  */
  class ProxyServer[Request,Reply](req: OutPort[Request], rep: InPort[Reply]) 
  extends 
        Server[Request,Reply]      
  with 
        OutPort.Proxy[Request]     
  with 
        InPort.Proxy[Reply]        
        {
           val inport  = rep
