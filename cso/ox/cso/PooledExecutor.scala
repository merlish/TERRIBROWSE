...
/**
        ...
        set to that number of seconds; if it is set, but is not a number, then 
        a default period is used. 
*/
...
{  
   ...
   */
   private def removeFromPool(server: Server)
   {   // Servers depart the pool at quiet times
       // So this is not as bad a bottleneck as it looks
       pool synchronized { pool.dequeueFirst((_==server)); () } 
   }
     ...
     { try
       { 
         ...
         { uses += 1
           ...
           pool synchronized { pool.enqueue(this) } // signal readiness for work
